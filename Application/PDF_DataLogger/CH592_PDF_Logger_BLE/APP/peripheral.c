...
// GAP GATT Attributes
static uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "Temp&Humi Logger";
...
uint16_t Peripheral_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & SBP_ENABLE_ADV_EVT)
    {
        uint8_t  initial_advertising_enable = TRUE;

        // Set the GAP Role Parameters
        GAPRole_SetParameter(GAPROLE_ADVERT_ENABLED, sizeof(uint8_t), &initial_advertising_enable);
        return (events ^ SBP_ENABLE_ADV_EVT);
    }

    if(events & SBP_DISABLE_ADV_EVT)
    {
        uint8_t  initial_advertising_enable = FALSE;

        // Set the GAP Role Parameters
        GAPRole_SetParameter(GAPROLE_ADVERT_ENABLED, sizeof(uint8_t), &initial_advertising_enable);
        return (events ^ SBP_DISABLE_ADV_EVT);
    }

    if(events & SBP_UPDATE_SCAN_RESPONSE_EVT)
    {
        tmos_memcpy(&scanRspData[22], &AHT20_TemperatureValue, 4);
        tmos_memcpy(&scanRspData[26], &AHT20_HumidityValue, 4);

        GAP_UpdateAdvertisingData(Peripheral_TaskID, FALSE, sizeof(scanRspData), scanRspData);

        return (events ^ SBP_UPDATE_SCAN_RESPONSE_EVT);
    }
